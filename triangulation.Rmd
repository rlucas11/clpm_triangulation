...
Indeed, this example provides hints as to why the reversal test of spuriousness fail. Consider the equation for the regression coefficient for the reversal test (predicting *Y1* from *X* controlling for *Y2*): $$\beta_{1} = \frac{r_{y1.x} - r_{y1.y2} \times r_{x.y2}} {1 - r_{x.y2}^{2}}$$ In this reversal-test equation, $r_{y1.x}$ is the correlation between *X* and *Y1*, $r_{y2.y1}$ is the correlation between *Y1* and *Y2* (the stability of Y), and $r_{x.y2}$ is the correlation between *X* and *Y2* (or the correlation between the two predictors in the model for the reversal test). 
...
Figure \@ref(fig:sim2) shows the results for the *difference* test proposed by the proponents of the triangulation method, in which the difference between *Y2* and *Y1* is regressed on *X*. In this case, the prediction is that for true effects, this coefficient should be positive. We spend less time discussing these results for three reasons. First, in their description of the triangulation method, the authors provide much less detail about the derivation of this test and the logic behind it. Second, the utility of this gain-score approach for examining causal questions was thoroughly discussed by @kim_gain_2021. They noted that although the coefficient that Sorjonen, Meli, and co-authors propose as being diagnostic of a true causal effect can be used in this way, this is true only under very restrictive assumptions. For instance, if we assume that *Y1* has a causal effect on *Y2* (an assumption that is common in situations where the CLPM would be used), then the gain-score approach does not recover unbiased estimates of the causal effect. The third reasons is that, the simulations shown in Figure \@ref(fig:sim2) show that, as with the reversal test, negative effects can often emerge even in the presence of true causal effects. Indeed, some of the factors that lead to these negative coefficients (smaller true causal effects of *X* and weoaker stability of *Y*) are those that lead to estimates that contradict predictions for the reversal test, too. Unlike the reversal test, increasing residual variance in *Y2* does not lead to higher likelihood of tests that are incorrectly flagged as being spurious. 
